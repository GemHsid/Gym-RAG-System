...
DEFAULT_DATA_DIR = Path("/data/rag_markdown_knowledge")
DEFAULT_VECTOR_DB = Path("/data/advanced_rag_fitness.db")

DATA_DIR = Path(get_env_str("RAG_DATA_DIR", str(DEFAULT_DATA_DIR)))
VECTOR_DB_PATH = Path(get_env_str("RAG_VECTOR_DB", str(DEFAULT_VECTOR_DB)))
EMBED_PATH = get_env_str("RAG_EMBED_PATH", "/mnt/moark-models/Qwen3-Embedding-8B")
LLM_MODEL = get_env_str("RAG_LLM_MODEL", "Qwen3-8B")
LLM_API_BASE = get_env_str("RAG_LLM_API_BASE", "http://127.0.0.1:8188/v1")
SERVER_HOST = get_env_str("RAG_SERVER_HOST", "0.0.0.0")
SERVER_PORT = int(get_env_str("RAG_SERVER_PORT", "8080"))
TOP_K = int(get_env_str("RAG_TOP_K", "3"))
VECTOR_DIM = int(get_env_str("RAG_VECTOR_DIM", "4096"))
...
def build_related_questions(query: str) -> list[str]:
    ...
    if any(word in query for word in ["营业", "时间", "洗澡", "停车", "wifi", "Wi-Fi"]):
        candidates.extend(
            [
                "健身房是24小时营业吗？",
                "有洗澡和更衣室吗？",
